...
export default async function Footer({ site }: { site: SiteInfo }) {
  // Derived from the live catalogue so renaming or removing a fabric in the
  // admin panel can never leave a dead link in the footer.
  const fabricLinks = (await getFeaturedProducts()).map((p) => ({
    label: p.name,
    href: `/fabrics/${p.slug}`,
  }));
