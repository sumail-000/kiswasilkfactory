...
import type { GalleryItem as ContentGalleryItem, Product, SiteInfo } from "@/lib/content";
...
type GalleryItem = ContentGalleryItem;

/* ─── WhatsApp ask button ─────────────────────────────────── */
function AskBtn({ label, phoneIntl }: { label: string; phoneIntl: string }) {
  return (
    <a
      href={`https://wa.me/${phoneIntl}?text=${encodeURIComponent(`Hi, I want to ask about: ${label}.`)}`}
      target="_blank"
        ...
        <div className="flex items-center justify-between">
          ...
          {item.ask && (
            <div onClick={(e) => e.stopPropagation()}>
              <AskBtn label={item.label} phoneIntl={phoneIntl} />
            </div>
          )}
        </div>
      </div>
