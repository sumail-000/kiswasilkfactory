...
import { getProduct, getProducts, getSite, type Product } from "@/lib/content";

/**
 * Products are editable at runtime, so the set of valid slugs is not fixed at
 * build time. `dynamicParams` must stay true — with it false, any fabric added
 * through the admin panel would 404 until the next deploy.
 */
...
export default async function FabricDetailPage({
  ...
  const { slug } = await params;
  const [p, site] = await Promise.all([getProduct(slug), getSite()]);
  if (!p) notFound();
